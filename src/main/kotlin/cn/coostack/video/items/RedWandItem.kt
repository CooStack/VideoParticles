...
class RedWandItem : Item(
    ...
    private fun handRelease(world: ServerWorld, user: PlayerEntity) {
        ...
        val v = user.rotationVector.normalize().multiply(-4.0)
        user.velocity = v
        ...
        spawnExplodesWithDamage(world, user)
    }
    ...
    private fun spawnReleaseParticles(world: ServerWorld, user: PlayerEntity) {
        ...
        val pos = user.eyePos
        val dir = user.rotationVector.normalize().multiply(5.0)
        TickHelper.runTaskTimerMaxTick(60) {
            for (i in 0..<10) {
                val to = rounds.random()
                ServerParticleUtil.spawnSingle(
    ...
    private fun spawnExplodesWithDamage(world: World, user: PlayerEntity) {
        val pos = user.eyePos
        for ((current, loc) in Math3DUtil.getLineLocations(
            pos, pos.add(user.rotationVector.normalize().multiply(120.0)), 6
        ).withIndex()) {
            ...
            world.getEntitiesByClass(
                ...
                it.damage(user.damageSources.mobAttack(user), 20f)
            }
        }
    }
